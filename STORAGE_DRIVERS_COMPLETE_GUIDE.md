...
### نسخه 1.1.0 (تاریخ فعلی)

#### ✅ اصلاحات و بهبودها

**1. سیستم انتخاب درایورها:**
- ✅ **رفع مشکل:** درایور پیش‌فرض (local) اکنون به صورت خودکار در تمام dropdown ها انتخاب می‌شود
- ✅ **بهبود:** فقط درایورهای فعال (enabled) در لیست‌های انتخاب نمایش داده می‌شوند
- ✅ **قاعده:** درایور local همیشه در دسترس و به عنوان پیش‌فرض انتخاب است

**2. سیستم احراز هویت تلگرام:**
- ✅ **افزوده شد:** UI کامل برای احراز هویت تلگرام با جریان خودکار
- ✅ **مراحل احراز هویت:**
  * **گام 1 (Idle):** دکمه "ورود به تلگرام" برای شروع احراز هویت
  * **گام 2 (Code):** ورود کد تأیید ارسال شده به تلگرام
  * **گام 3 (2FA):** ورود رمز عبور دو مرحله‌ای (در صورت فعال بودن)
  * **گام 4 (Authenticated):** تست کانال پس از ورود موفق
- ✅ **دکمه‌های جدید:**
  * "ورود به تلگرام" - شروع احراز هویت
  * "تأیید کد" - تأیید کد ارسالی
  * "تأیید رمز عبور" - تأیید 2FA
  * "تست کانال" - تست ارتباط با کانال
  * "بازنشانی ورود" - شروع مجدد فرآیند احراز هویت
  * "لغو" - انصراف از مرحله فعلی

**3. تغییرات فایل‌ها:**

**Frontend:**
- `admin/src/components/settings/storage/index.tsx`:
  * اضافه شدن state های احراز هویت تلگرام
  * پیاده‌سازی handler های مربوط به احراز هویت
  * اضافه شدن UI گام‌به‌گام برای ورود
  * فیلتر داینامیک درایورهای فعال
  * تنظیم خودکار defaultValue به local

- `admin/public/locales/fa/form.json`:
  * اضافه شدن 17 ترجمه جدید برای سیستم احراز هویت تلگرام
  * پیام‌های خطا و موفقیت
  * راهنماهای گام‌به‌گام

**تنظیمات:**
- `admin/.env`:
  * ایجاد فایل .env با تنظیمات صحیح
  * اتصال به backend: https://srcchi.top/backend

**4. بهبود تجربه کاربری:**
- ✅ نمایش پیام‌های واضح در هر مرحله از احراز هویت
- ✅ نمایش خطاها به صورت inline در فیلدها
- ✅ امکان لغو و بازگشت در هر مرحله
- ✅ نمایش وضعیت موفق با Alert سبز
- ✅ غیرفعال شدن دکمه‌ها در حین پردازش (loading state)

**5. تغییرات Backend:**
- ℹ️ بدون تغییر - APIها از قبل به درستی پیاده‌سازی شده بودند

---

### نحوه استفاده از سیستم جدید

#### راه‌اندازی تلگرام (نسخه جدید)

1. **وارد Admin Panel شوید:**
   - Settings → Storage
   - فعال کردن "Telegram Storage"

2. **وارد کردن اطلاعات:**
   - API ID (از my.telegram.org)
   - API Hash (از my.telegram.org)
   - Phone Number (+989123456789)
   - Channel ID (@channel یا -100XXX)

3. **احراز هویت (جریان خودکار):**
   
   **گام اول - شروع ورود:**
   ```
   1. روی دکمه "ورود به تلگرام" کلیک کنید
   2. کد تأیید به تلگرام شما ارسال می‌شود
   ```

   **گام دوم - تأیید کد:**
   ```
   1. کد 5 رقمی را از تلگرام وارد کنید
   2. روی "تأیید کد" کلیک کنید
   ```

   **گام سوم - 2FA (در صورت نیاز):**
   ```
   1. رمز عبور دو مرحله‌ای را وارد کنید
   2. روی "تأیید رمز عبور" کلیک کنید
   ```

   **گام چهارم - تست کانال:**
   ```
   1. پس از ورود موفق، پیام موفقیت نمایش داده می‌شود
   2. روی "تست کانال" کلیک کنید
   3. سیستم یک فایل تستی آپلود کرده و حذف می‌کند
   ```

4. **ذخیره تنظیمات:**
   - روی دکمه "Save Settings" کلیک کنید
   - تنظیمات و session شما ذخیره می‌شود

#### نکات مهم

✅ **Session ماندگار:** پس از ورود موفق، session تا زمان logout باقی می‌ماند

✅ **بازنشانی ورود:** در صورت مشکل، روی "بازنشانی ورود" کلیک کنید

✅ **لغو در هر مرحله:** می‌توانید در هر مرحله روی "لغو" کلیک کنید

✅ **مشاهده خطاها:** تمام خطاها به زبان فارسی و واضح نمایش داده می‌شوند

---

### API Endpoints (بدون تغییر)

تمام endpoint های قبلی بدون تغییر باقی مانده‌اند:
- `POST /api/storage/telegram/auth/start`
- `POST /api/storage/telegram/auth/verify`
- `POST /api/storage/telegram/auth/2fa`
- `POST /api/storage/telegram/test-channel`
