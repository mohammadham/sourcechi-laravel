...
class ProductController extends CoreController
{
    ...
    public function fetchProducts(Request $request)
    {
        ...
        // Use the new forLanguage scope to filter products
        ...
        if ($request->has('tags')) {
            ...
            $products_query = $products_query->whereHas('tags.translations', function ($q) use ($tags, $language) {
                $q->where('language', $language)->whereIn('slug', $tags);
            })->orWhereHas('tags', function ($q) use ($tags, $language) {
                // تگ‌هایی که ترجمه ندارن ولی اسلاگشون در زبان پایه match داره
                $q->whereDoesntHave('translations')->whereIn('slug', $tags);
            });
        }
...
public function scopeForLanguageWithTags($query, $language)
{
    return $query->whereHas('tags.translations', function ($q) use ($language) {
        $q->where('language', $language);
    })->orWhereHas('tags', function ($q) use ($language) {
        // تگ‌هایی که ترجمه ندارن ولی به زبان پایه هستن
